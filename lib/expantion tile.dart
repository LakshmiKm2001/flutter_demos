
import 'package:device_preview/device_preview.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';

void main() {
  runApp(DevicePreview(
      enabled: !kReleaseMode,
      builder: (context) => MaterialApp(
        debugShowCheckedModeBanner: false,
            home: expantion_tile(),
            useInheritedMediaQuery: true,
            locale: DevicePreview.locale(context),
            builder: DevicePreview.appBuilder,
          )));
}

class expantion_tile extends StatelessWidget {

  var colors=[Colors.red,Colors.blue,Colors.green,Colors.purple,Colors.pink];
  List datas=["RED","BLUE","GREEN","PURPLE","PINK"];


  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("EXPANSION TILE"),),
      body: ExpansionTile(
        title: Text("Colors"),
        subtitle: Text("List of Colors"),
        children: List.generate(5, (index) => ListTile(
            leading: CircleAvatar(backgroundColor: colors[index],),
            title: Text(datas[index]),
        )),
      ),
    );
  }
}